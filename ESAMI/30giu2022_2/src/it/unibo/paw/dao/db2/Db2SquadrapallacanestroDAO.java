...
public class Db2SquadrapallacanestroDAO implements SquadrapallacanestroDAO {
	...
	static final String insert = "INSERT " + "INTO " + TABLE + " ( " + NOME + ", " + TORNEO + ", " + ALLENATORE + ") "
			+ "VALUES (?,?,?) ";
...
// ------------------- METODI DAO -----------------------
	public void create(SquadrapallacanestroDTO squadrapallacanestro) {
		...
		try {
			PreparedStatement prep_stmt = conn.prepareStatement(insert, Statement.RETURN_GENERATED_KEYS);
			...
			ResultSet rs = prep_stmt.getGeneratedKeys();

			if (rs.next()) {
				squadrapallacanestro.setId(rs.getInt(1));
			}
...
// -------------------------------------------------------------------------------------
	...
	public boolean update(SquadrapallacanestroDTO squadrapallacanestro) {
		...
		try {
			...
			prep_stmt.clearParameters();
			prep_stmt.setInt(1, squadrapallacanestro.getId());
